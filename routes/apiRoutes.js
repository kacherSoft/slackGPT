...
router.post('/correct', async (req, res) => {
  const input = req.body.text;
    ...
    try {
      const correctedText = await correctText(input);
...
router.post('/rewrite', async (req, res) => {
  ...
  // Step 2: Process the GPT-3 correction in the background
  try {
    const correctedText = await rewriteText(input);
...
router.post('/shorten', async (req, res) => {
  ...
  // Step 2: Process the GPT-3 correction in the background
  try {
    const correctedText = await shortenText(input);
...
router.post('/longer', async (req, res) => {
  ...
  // Step 2: Process the GPT-3 correction in the background
  try {
    const correctedText = await longerText(input);
